...
for epoch in range(num_epochs):
    ...
    for i in range(len(val_data)):
        ...
        cost_test, dumb, correct_test = cross_entropy(y_label = val_label[i], out = out_layer.output, reg_loss = out_layer.reg_loss)
        total_cost_test += cost_test
        total_acc_test += correct_test
    print("Validation loss at epoch#{}: {}".format(epoch, total_cost_test/len(val_data)))
    print("Validation accuracy at epoch#{}: {}".format(epoch, total_acc_test/len(val_data)))
    costs_test.append(total_cost_test/len(val_data))
    acc_test.append(total_acc_test/len(val_data))

# Plotting-----------------------------------------------------------
fig1, ax1 = plt.subplots()
ax1.plot(range(num_epochs), costs, label = "Training Loss")
ax1.plot(range(num_epochs), costs_test, label = "Validation Loss")
...
ax2.plot(range(num_epochs), acc, label = "Training Accuracy")
ax2.plot(range(num_epochs), acc_test, label = "Validation Accuracy")
